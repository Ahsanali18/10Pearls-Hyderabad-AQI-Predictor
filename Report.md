...
**Live Dashboard:** 🔗 [Streamlit App](https://10pearls-hyderabad-aqi-predictor-xgct4ppjp9s5qmyqucwxzv.streamlit.app/)
...
**Why mean AQI, not hourly:** Predicting a 24-hour mean reduces the impact of transient hour-to-hour fluctuations, produces smoother and more actionable forecasts for residents (people plan their day, not individual hours), and makes the model's task more tractable at longer horizons where hour-level precision is physically unrealistic.
...
Each model is trained and evaluated on only the features relevant to its horizon. The live pipeline's  applies the same filtering at inference time — the feature set at prediction time always matches what the model was trained on.
...
**Training (Historical data — `engineer_features()`):**
...
**Inference (live hourly — `compute_live_features()`):**
...
- Targets are NaN (unknown by definition at inference time)
...
The progression from 24h to 72h: shallower trees (max_depth 5→3), fewer estimators (500→200), stronger regularization (reg_lambda 1.0→5.0). This directly counteracts the overfitting risk that grows as the target becomes harder to predict.
...
A comprehensive EDA was conducted in `notebooks/EDA.ipynb` to ground every feature engineering decision in data evidence:
...
6. **Strong Seasonal Variation** — Winter mean=106.3 vs Monsoon mean=74.3 — a 32-point gap. This was the direct quantitative evidence that 6 months of data was insufficient for reliable multi-day forecasting.

7. **No Weekend Effect** — Weekday 86.4 vs Weekend 87.6 — only 1.2 point difference. AQI in Hyderabad is driven by regional atmospheric conditions, not local traffic patterns. `is_weekend` retained as a feature despite weak signal.
...
**Day 1 (24h):** Short-lag features dominate — `aqi_lag_1h`, `aqi_lag_3h`, `aqi_rolling_mean_24h`. Current PM2.5 and current weather conditions appear in top 10. The next 24h mean is primarily determined by where AQI is right now.
...
**Day 3 (72h):** Long-lag and weather forecast features dominate — `aqi_lag_48h`, `aqi_lag_72h`, `temp_72h`, `wind_72h`, `pressure_72h`, `pm2_5_72h`. Short-lag features (correctly excluded via IRRELEVANT_72H) would have appeared near-zero if included, confirming the horizon-aware filtering made correct exclusions.
...
- **Feature storage:** All ingested weather and air quality data is stored in MongoDB collections (`aqi_features`, `weather_forecast`), with upsert logic on timestamps to avoid duplicates. This effectively acts as a feature store — the training pipeline reads directly from these collections.
...
The results were devastating. Four compounding problems made this approach unworkable:
...
2. **Lag features drowned out everything else.** The model essentially learned *"PM2.5 tomorrow ≈ PM2.5 today"* — weather features (temperature, wind speed) and time features barely contributed. There was no meaningful multi-day forecasting happening.
...
4. **EPA breakpoint amplification.** Because of the breakpoint structure, a small PM2.5 error near a category boundary could cause the AQI to jump by 20–30 points, making an already-bad model look catastrophically wrong.
...
| Attribute | Value |
|---|---|
| Date range | January 4, 2025 → May 27, 2026 |
...
- **Open-Meteo forecast quality:** At t+72h, the Open-Meteo weather forecast itself has uncertainty. Our model's accuracy is bounded by the quality of the forecast weather inputs it receives.
...
- Expand to other cities like Karachi, Lahore, Islamabad of Pakistan.
...
*Live Dashboard: https://10pearls-hyderabad-aqi-predictor-xgct4ppjp9s5qmyqucwxzv.streamlit.app/*
